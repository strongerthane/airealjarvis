...
export function createOrbScene(container: HTMLElement): OrbSceneApi {
  ...
  // ——— COLORS (blue palette) ———
  const C_BRIGHT = 0x7dd3fc; // light cyan
  const C_MID = 0x3b82f6; // blue-500
  const C_DIM = 0x1e40af; // blue-900
  const C_FAINT = 0x062e49; // very dark blue
  const C_HOT = 0x93c5fd; // pale blue/white
  ...
  // CODE TEXT, DEBRIS, DUST etc. (omitted for brevity: reusing same approach as original)
  // For brevity and maintainability, recreate a simplified text/debris/dust setup that's efficient.
  ...
  const textOuter = scatterText(300, () => 0.03 + Math.random() * 0.03, () => R1 + 0.03 + Math.random() * 0.08, [0.0003, 0.0006]);
  orbGroup.add(textOuter);

  // Debris simplified
  const debrisGeos = [new THREE.IcosahedronGeometry(0.012, 0), new THREE.TetrahedronGeometry(0.015, 0), new THREE.OctahedronGeometry(0.018, 0)];
  const debris: THREE.Mesh[] = [];
  for (let i = 0; i < 120; i++) {
    const geo = debrisGeos[Math.floor(Math.random() * debrisGeos.length)];
    const mat = new THREE.MeshBasicMaterial({ color: Math.random() > 0.6 ? C_BRIGHT : C_MID, transparent: true, opacity: 0.3 + Math.random() * 0.6, blending: THREE.AdditiveBlending });
  ...
  function animate() {
    ...
    const driftGroups: [THREE.Group, number][] = [[textOuter, 1], [/*textInner*/ textOuter, 2], [/*textAmbient*/ textOuter, 1.2]];
